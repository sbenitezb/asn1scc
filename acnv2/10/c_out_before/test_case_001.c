...
PDU_LENGTH_TYPE tmp0;
        PDU_LENGTH_TYPE_Initialize((&(tmp0)));
    }

PDU_LENGTH_TYPE tc_data; 
    tc_data = 0UL;
#ifndef _MSC_VER
    struct timeval stop, start;
    gettimeofday(&start, NULL);
    result = PDU_LENGTH_TYPE_ACN_enc_dec(&tc_data, &errCode, "test_case_ACN_000001");
    gettimeofday(&stop, NULL);
    if (result) {
        output->report_test_case_success("PDU_LENGTH_TYPE/test_case_ACN_000001", ((stop.tv_sec - start.tv_sec) * 1000000 + stop.tv_usec - start.tv_usec)/1000);
    } else {
#else
    result = PDU_LENGTH_TYPE_ACN_enc_dec(&tc_data, &errCode, "test_case_ACN_000001");
    if (!result) {
...
PDU_LENGTH_TYPE tc_data; 
    ...
    struct timeval stop, start;
    gettimeofday(&start, NULL);
    result = PDU_LENGTH_TYPE_ACN_enc_dec(&tc_data, &errCode, "test_case_ACN_000002");
    gettimeofday(&stop, NULL);
    if (result) {
        output->report_test_case_success("PDU_LENGTH_TYPE/test_case_ACN_000002", ((stop.tv_sec - start.tv_sec) * 1000000 + stop.tv_usec - start.tv_usec)/1000);
    } else {
#else
    result = PDU_LENGTH_TYPE_ACN_enc_dec(&tc_data, &errCode, "test_case_ACN_000002");
    if (!result) {
...
BUFFER_LENGTH_TYPE tc_data; 
    ...
    struct timeval stop, start;
    gettimeofday(&start, NULL);
    result = BUFFER_LENGTH_TYPE_ACN_enc_dec(&tc_data, &errCode, "test_case_ACN_000003");
    gettimeofday(&stop, NULL);
    if (result) {
        output->report_test_case_success("BUFFER_LENGTH_TYPE/test_case_ACN_000003", ((stop.tv_sec - start.tv_sec) * 1000000 + stop.tv_usec - start.tv_usec)/1000);
    } else {
#else
    result = BUFFER_LENGTH_TYPE_ACN_enc_dec(&tc_data, &errCode, "test_case_ACN_000003");
    if (!result) {
...
BUFFER_LENGTH_TYPE tc_data; 
    ...
    struct timeval stop, start;
    gettimeofday(&start, NULL);
    result = BUFFER_LENGTH_TYPE_ACN_enc_dec(&tc_data, &errCode, "test_case_ACN_000004");
    gettimeofday(&stop, NULL);
    if (result) {
        output->report_test_case_success("BUFFER_LENGTH_TYPE/test_case_ACN_000004", ((stop.tv_sec - start.tv_sec) * 1000000 + stop.tv_usec - start.tv_usec)/1000);
    } else {
#else
    result = BUFFER_LENGTH_TYPE_ACN_enc_dec(&tc_data, &errCode, "test_case_ACN_000004");
    if (!result) {
...
void test_case_ACN_000005(const TestOutput* output, int *totalErrors)
{
    ...
    i1 = 0;
    while (i1< 0) {
        tc_data.payload.buffer1.arr[i1] = (byte)(i1%256);
        i1 = i1 + 1;
    }
    tc_data.payload.buffer1.nCount = 0;

    /*set buffer2 */
    i1 = 0;
    while (i1< 0) {
        tc_data.payload.buffer2.arr[i1] = (byte)(i1%256);
        i1 = i1 + 1;
    }
    tc_data.payload.buffer2.nCount = 0;
...
void test_case_ACN_000006(const TestOutput* output, int *totalErrors)
{
    ...
    tc_data.payload.buffer1.nCount = 100;
    ...
    tc_data.payload.buffer2.nCount = 100;
